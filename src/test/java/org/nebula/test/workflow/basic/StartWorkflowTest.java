...
public class StartWorkflowTest extends BaseTest {
  ...
  public void test() {
    ...
    startWorkflowWorker(StartWorkflowImpl.class, realms);

    StartWorkflowClientImpl client = new StartWorkflowClientImpl(
        nebulaClient, workflowProfile, realms);

    String name = "<user name=\"nebula\">nebula-service</user>";
    client.start(name);

    log.info("instanceId=" + client.getInstanceId());

    this.waitForWorkflowCompletion(client.getInstanceId(), 20, 2);
